...
with torch.no_grad():
     key_points, r1= net(torch.FloatTensor(x).to(ns.device),'False')      
for kp in key_points:
    out_kpcd.append(kp)
for i in range(len(out_kpcd)):
    out_kpcd[i] = out_kpcd[i].cpu().numpy()
np.savetxt('Key_Grid/dataset/fold/pant_keypoint.txt', np.array(out_kpcd).reshape(-1,np.array(out_kpcd).shape[-1]))    
